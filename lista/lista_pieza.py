from nodo.nodo_pieza import nodo_pieza
from classs.pieza import pieza
...
class lista_piezas:
  ...
  def actualizar_pieza(self,fila,columna,color,simbolos):
    actual=self.primero
    while actual!=None:
      if actual.pieza.fila==fila and actual.pieza.columna==columna:
        actual.pieza.color=color
        actual.pieza.simbolos=simbolos
  ...
  def devolver_color_de_pieza(self,fila,columna):
    ...
    while actual!=None:
      if actual.pieza.fila==fila and actual.pieza.columna==columna:
        return actual.pieza.simbolos
      actual=actual.siguiente

  def devolver_color_de_pieza_English(self,fila,columna):
    actual=self.primero
    while actual!=None:
      if actual.pieza.fila==fila and actual.pieza.columna==columna:
  ...
  def graficar(self):
    ...
    for i in range(1, self.filas + 1):
      texto+="<tr>\n"
      for j in range(1, self.columnas + 1):
        texto+="<td bgcolor=\""+self.devolver_color_de_pieza_English(i,j)+"\" width=\"1\" height=\"1\">"+str(i)+","+str(j)+"</td>\n"
      texto+="</tr>\n"
    texto+="</table>>\n];\n}"
    f.write(texto)
    f.close()
